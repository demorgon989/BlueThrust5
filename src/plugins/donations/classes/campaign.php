...
class DonationCampaign extends Basic {
	...
	private function logDebug($message) {
		...
		@file_put_contents($logFile, "[".$timestamp."] ".trim($message)."\n", FILE_APPEND);
	...
	public function populateDonationInfo($total = false, $currentPeriod = 0, $nextPeriod = 0) {
		...
		if ($this->intTableKeyValue != "") {
			...
			$addSQL = "";
			if (!$total) {
				if (count($arrPeriod) > 0) {
					$sqlCurrentPeriod = ($currentPeriod == 0) ? $arrPeriod['current'] : $currentPeriod;
					$sqlNextPeriod = ($nextPeriod == 0) ? $arrPeriod['next'] : $nextPeriod;
					$addSQL = " AND datesent >= '".$sqlCurrentPeriod."' AND datesent < '".$sqlNextPeriod."'";
				} elseif ($currentPeriod != 0 || $nextPeriod != 0) {
					$sqlCurrentPeriod = ($currentPeriod == 0) ? 0 : $currentPeriod;
					$sqlNextPeriod = ($nextPeriod == 0) ? time() : $nextPeriod;
					$addSQL = " AND datesent >= '".$sqlCurrentPeriod."' AND datesent < '".$sqlNextPeriod."'";
				}
			}
			$query = "SELECT donation_id, donationcampaign_id, member_id, name, message, datesent, amount, hideamount, paypalemail, transaction_id, response FROM ".$this->MySQL->get_tablePrefix()."donations WHERE donationcampaign_id = '".$this->intTableKeyValue."'".$addSQL." ORDER BY transaction_id, datesent DESC";
			$this->logDebug("populateDonationInfo query: ".$query);
			$result = $this->MySQL->query($query);
			$totalDonationAmount = 0;
			if ($result !== false) {
				$this->logDebug("populateDonationInfo rows returned: ".$result->num_rows);
				$seenTransactions = [];
				while ($row = $result->fetch_assoc()) {
					$transactionKey = $row['transaction_id'];
					if ($transactionKey != "" && isset($seenTransactions[$transactionKey])) {
						continue;
					}
					if ($transactionKey != "") {
				...
				$this->logDebug("populateDonationInfo query failed: ".$this->MySQL->error);
			}
	...
	public function getDonators($allTime = false) {
		...
		if ($this->intTableKeyValue != "") {
			...
			$query = "SELECT donation_id, donationcampaign_id, member_id, name, message, datesent, amount, hideamount, paypalemail, transaction_id, response FROM ".$this->MySQL->get_tablePrefix()."donations WHERE donationcampaign_id = '".$this->intTableKeyValue."'".$addSQL." ORDER BY transaction_id, datesent DESC";
			$this->logDebug("getDonators query: ".$query);
			$result = $this->MySQL->query($query);
			if ($result !== false) {
				$this->logDebug("getDonators rows returned: ".$result->num_rows);
				$seenTransactions = [];
				while ($row = $result->fetch_assoc()) {
					$transactionKey = $row['transaction_id'];
					if ($transactionKey != "" && isset($seenTransactions[$transactionKey])) {
				...
				$this->logDebug("getDonators query failed: ".$this->MySQL->error);
			}
		}
		return $returnVal;
	}
...
";
	...
	public function getCurrencySymbol() {
		...
		if ($this->intTableKeyValue != "") {
			...
			if (@include(BASE_DIRECTORY."plugins/donations/include/currency_codes.php")) {
				$currencyInfoArray = $arrPaypalCurrencyInfo ?? [];
			}
			$currency = $this->arrObjInfo['currency'] ?? 'USD';
			$defaultCurrency = ['symbol' => '$', 'position' => 'left'];
			$currencyInfo = $currencyInfoArray[$currency] ?? $currencyInfoArray['USD'] ?? $defaultCurrency;
	...
	public static function getCurrencyCodes() {
		$currencyCodes = [];
		if (@include(BASE_DIRECTORY."plugins/donations/include/currency_codes.php")) {
			$currencyCodes = $arrPaypalCurrencyCodes ?? [];
		}
	...
	public static function getCurrencyCodeInfo() {
		$currencyInfo = [];
		if (@include(BASE_DIRECTORY."plugins/donations/include/currency_codes.php")) {
			$currencyInfo = $arrPaypalCurrencyInfo ?? [];
		}
		return $currencyInfo;
	}
}
